...
**Source:** `../workflow/product_requirements_clarified.md` (authoritative). The Part 1 HTTP API lives under `src/` and is **not** modified for this task except as needed for unrelated fixes (none required). Paths below for the CLI are under **`cli/`**.

## Primary goal

**Go** CLI (**HTTP client only**) calling the Blueprint Manager API at **`{base}/blueprints`**. No Postgres. Success JSON to **stdout**; errors and API error bodies to **stderr** per conventions below. **`../bricks.json`** (from `cli/` working directory: `bricks.json`) is the canonical sample for **create** / **update** in README examples.

## Deliverables

1. Go module at **`cli/`** with **`go.mod`** (Go **≥ 1.22**).
